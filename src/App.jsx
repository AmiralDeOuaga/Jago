...
export default function YoMan() {
  ...
  const {
    ...
    rWa, setRWa,
    rPwd, setRPwd,
    loginGoogle, forgotPassword, login, register,
  } = useAuth(showToast);
    ...
    const initPush = async () => {
      try {
        ...
        PushNotifications.addListener("registration", async (token) => {
          try { await updateDoc(doc(db, "users", user.uid), { fcmToken: token.value }); }
          catch (e) { await setDoc(doc(db, "users", user.uid), { fcmToken: token.value }, { merge: true }); }
        });
