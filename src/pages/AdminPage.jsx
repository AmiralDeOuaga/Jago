import { categories, getPays } from "../constants";
    ...
    <div className="app"><Header/>
      <div className="admin-screen">
        <div className="admin-header">
          <div>
            <div className="admin-title">Panel Admin — Jago</div>
        ...
        <div className="admin-tabs">
          ...
          <button className={`admin-tab${adminTab==="demandes"?" on":""}`} onClick={()=>setAdminTab("demandes")}>Demandes {changeRequests.length > 0 ? `(${changeRequests.length})` : ""}</button>
        ...
        {adminTab==="annonces" && (
          <div className="admin-section">
            ...
            {annonces.length===0
              ? <div className="empty"><div className="emsg">Aucune annonce</div></div>
              : annonces.map(a=>(
                <div key={a.id} className="admin-row">
                  <div className="admin-row-info">
                    <div className="admin-row-title">{a.urgent && "[Urgent] "}{a.titre}</div>
                    <div className="admin-row-sub">{categories.find(c=>c.id===a.categorie)?.label} · {a.ville} · {a.vendeur} · {a.vues||0} vues</div>
                    <div className="admin-row-sub" style={{color:"var(--blue)",fontWeight:700}}>{a.prix}</div>
                  </div>
                  <div className="admin-row-actions">
                    <button className="btn-danger" onClick={()=>adminDeleteAd(a.id)}>Supprimer</button>
                  </div>
                </div>
              ))
            }
        ...
        {adminTab==="signalements" && (
          <div className="admin-section">
            ...
            {signalements.length===0
              ? <div className="empty"><div className="emsg">Aucun signalement !</div></div>
              : signalements.map(s=>(
                <div key={s.id} className="admin-row">
                  ...
                  <div className="admin-row-actions">
                    <button className="btn-danger" onClick={()=>adminDeleteAd(s.annonceId)}>Supprimer l'annonce</button>
        ...
        {adminTab==="users" && (
          <div className="admin-section">
            ...
            {allUsers.length===0
              ? <div className="empty"><div className="emsg">Aucun membre</div></div>
              : allUsers.map(u=>(
        ...
        {/* Demandes changement de pays */}
        {adminTab==="demandes" && (
          <div className="admin-section">
            <div className="admin-section-title">Demandes de changement de pays</div>
            {changeRequests.length === 0
              ? <div className="empty"><div className="emsg">Aucune demande en attente</div></div>
              : changeRequests.map(r => (
                <div key={r.uid} className="admin-row">
                  <div className="admin-row-info">
                    <div className="admin-row-title">{r.pseudo || r.email}</div>
                    <div className="admin-row-sub" style={{display:"flex",alignItems:"center",gap:6}}>
                      <img src={`https://flagcdn.com/w20/${r.currentPays}.png`} alt="" style={{width:16,height:11,objectFit:"cover",borderRadius:2}}/>
                      {getPays(r.currentPays).label}
                      <span style={{color:"var(--muted)"}}>→</span>
                      <img src={`https://flagcdn.com/w20/${r.requestedPays}.png`} alt="" style={{width:16,height:11,objectFit:"cover",borderRadius:2}}/>
                      {getPays(r.requestedPays).label}
                    </div>
                    <div className="admin-row-sub">{r.email}</div>
                  </div>
                  <div className="admin-row-actions">
                    <button className="btn-warn" style={{background:"rgba(34,197,94,.1)",color:"#4ade80",borderColor:"rgba(34,197,94,.3)"}} onClick={()=>approveChangeRequest(r)}>Approuver</button>
                    <button className="btn-danger" onClick={()=>rejectChangeRequest(r)}>Refuser</button>
                  </div>
                </div>
              ))
            }
